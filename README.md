...
本文档说明 PkkMind 的部署、启动、API 调用、知识库使用、ChromaDB 数据查看、监控评测和常见排障。

PkkMind 是一个企业级智能客服系统，核心链路为：
...
PkkMind 常用两种 Docker 启动方式：`docker compose up` 全栈部署，以及 `docker run` 开发模式。两者最大的区别是：**全栈部署会同时启动应用和依赖服务；run 开发模式通常只手动运行一个应用容器，依赖服务需要提前启动**。

| 对比项 | Docker Compose 全栈部署 | Docker run 开发模式 |
|--------|--------------------------|----------------------|
| 启动命令 | `docker compose up -d --build` | `docker run ... pkkmind ...` |
| 启动内容 | PkkMind、Redis、ChromaDB、Prometheus、Nginx | 只启动你指定的单个容器 |
| Redis/ChromaDB | 自动启动并加入同一网络 | 必须先执行 `docker compose up -d redis chromadb` |
| 容器网络 | Compose 自动创建并管理 | 需要手动指定 `--network pkkmind_pkkmind-network` |
...
- 如果只是跑 CLI，最省心的方式是 `docker compose run --rm pkkmind python api/main.py --cli`，它会自动使用 Compose 网络。
...
```bash
docker compose logs -f pkkmind
```

看到 PkkMind 启动日志并且健康检查通过后，服务可用。

启动后的端口：

| 服务 | 容器名 | 宿主机端口 | 容器内端口 | 用途 |
|------|--------|------------|------------|------|
| PkkMind API | `pkkmind-app` | `8000` | `8000` | 主 API 服务 |
| Nginx | `pkkmind-nginx` | `80` | `80` | 反向代理 |
| ChromaDB | `pkkmind-chromadb` | `8001` | `8000` | 向量数据库 |
| Redis | `pkkmind-redis` | `6379` | `6379` | 工作记忆 |
| Prometheus | `pkkmind-prometheus` | `9090` | `9090` | 监控数据 |
...
```bash
docker compose build --no-cache pkkmind
```
...
```bash
docker run -it --rm \
  --network pkkmind_pkkmind-network \
  -p 8000:8000 \
  -e ANTHROPIC_BASE_URL="https://api.deepseek.com/anthropic" \
  -e ANTHROPIC_API_KEY="your_key" \
  -e ANTHROPIC_MODEL="deepseek-v4-pro" \
  -e REDIS_URL="redis://:pkkmind123@redis:6379/0" \
...
PkkMind 基于 FastAPI 构建，启动 HTTP 服务后可以直接在浏览器访问 Swagger UI 调用接口。
...
PkkMind 支持从目录加载 Skills，用来把业务流程、客服话术、排障 SOP 等规则动态注入 Agent。

默认配置：

```env
PKKMIND_SKILLS_DIR=./skills
PKKMIND_SKILLS_MAX_PROMPT_CHARS=5000
```
...
PkkMind 的知识库由 `mcp/knowledge_base.py` 管理，底层使用 ChromaDB collection：
...
PkkMind 使用了三个 ChromaDB collection：
...
```bash
docker exec -it pkkmind-chromadb curl http://localhost:8000/api/v1/heartbeat
```
...
```bash
docker exec -it pkkmind-app bash
```
...
docker volume inspect pkkmind_chromadb-data
```

查看容器内数据目录：

```bash
docker exec -it pkkmind-chromadb sh
...
```bash
docker compose down
docker volume rm pkkmind_chromadb-data
docker compose up -d --build
```
...
```bash
docker exec -it pkkmind-redis redis-cli -a pkkmind123
```
...
```bash
docker compose restart pkkmind
```
...
```bash
docker exec -it pkkmind-redis redis-cli -a pkkmind123 ping
```
...
2. 等待几秒
3. 查看 `docker compose logs -f pkkmind` 是否出现 `用户画像已更新`
...
# 6. 检索
curl -X POST "http://localhost:8000/search?query=PkkMind如何接入API&top_k=3"
